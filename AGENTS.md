...
- Keep Python compatibility at 3.12 or newer and keep Gemini SDK code inside `src/gemini_injection_lab/client.py`.
...
- Tool definitions must remain JSON Function Declarations. Never add Python callables, dispatchers, Function Results, tool execution, agent loops, Gmail, Drive, SMTP, shell, subprocess, `os.system`, `eval`, or `exec`.
