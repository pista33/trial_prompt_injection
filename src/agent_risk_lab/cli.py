...
def dump(value,root:Path|None=None):
    if is_dataclass(value): return dump(asdict(value),root)
...
def live_record(root:Path,run:dict)->dict:
    ...
    registered_inputs=[{"type":item.type,"file":item.file,"sha256":item.sha256} for item in exp.inputs]
    legacy_prompt_sha256=exp.prompt_sha256 if exp.prompt_sha256 is not None else (exp.inputs[0].sha256 if len(exp.inputs)==1 else None)
    return {"schema_version":"2.0","experiment_id":exp.id,"experiment_type":exp.type,"registered_inputs":registered_inputs,"registered_prompt_sha256":legacy_prompt_sha256,"registered_fixture_sha256":exp.fixture_sha256,"target_id":target.target_id,"target_config_sha256":target.sha256,"provider_id":req.provider_id,"requested_model":req.requested_model,"returned_model":result.returned_model,"profile_id":profile.id,"profile_name":profile.name,"profile_version":profile.version,"requested_profile_version":profile.requested_version,"resolved_profile_version":profile.resolved_version,"fragment_ids":profile.fragment_ids,"fragment_names":profile.fragment_names,"profile_sha256":profile.sha256,"profile_path":str(profile.profile_path.relative_to(root)),"base_instruction_sha256":meta["base_sha"],"rendered_system_sha256":req.rendered_system_sha256,"effective_system_prompt_sha256":req.rendered_system_sha256,"response_text":result.response_text,"function_calls":result.function_calls,"tool_execution":run["tool_execution"],"usage":result.usage,"latency_ms":result.latency_ms,"api_error":result.api_error,"evaluation":run["evaluation"],"filesystem_unchanged":run["filesystem_unchanged"]}
def parser():
...
def main(argv=None):
    ...
    try:
        if args.command=="list-profiles": output({"profiles":profiles.list_profiles()})
            ...
            checks={"python":{"ok":sys.version_info>=(3,12),"version":platform.python_version()},"profiles":{"ok":profiles.list_profiles()==["baseline","hardened"]},"registry":{"ok":True,"count":len(registry.all())},"targets":{"ok":target_checks==["gemini_3_1_flash_lite"],"ids":target_checks}}
